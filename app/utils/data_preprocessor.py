...
import nltk
import re
nltk.data.path.append('/Users/rakumar/Projects/ChromaDB/app/nltk_data') ## Add relative Path
...
class TextPreprocessorBuilder:
    ...
    # _stop_words = set(stopwords.words('english')) # from nltk Library, under nltk_data dir
    ...
    #     Convert the tokens(excluding abbreviation or constant) to lowercase.
    #     '''
    ...
    #     STEP: 8a
    #     Breaks the text into tokens.
    #     Checks each token to see if its a number
    #     If the number has at least min_len digits, it converts it into words using num2words
    #     '''
    #     # Match both words and non-word characters
    #     tokens = re.findall(r'\b\w+\b|\W+', self.text)
    #     for i, token in enumerate(tokens):
    ...
    #     '''
    #     STEP: 4
    #     Remove common stopwords from the text using a predefined stopword list.
    #     '''
